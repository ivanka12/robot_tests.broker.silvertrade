...
def convert_datetime_to_silvertrade_format(isodate):
    iso_dt = parse_date(isodate)
    day_string = iso_dt.strftime("%d/%m/%Y %H:%M")
    return day_string


def convert_string_from_dict_silvertrade(string):
    return {
        u"грн": u"UAH",
...
def adapt_view_data(value, field_name):
    ...
    return convert_string_from_dict_silvertrade(value)
...
def get_upload_file_path():
    return os.path.join(os.getcwd(), 'src', 'robot_tests.broker.silvertrade', 'testFileForUpload.txt')


def silvertrade_download_file(url, file_name, output_dir):
